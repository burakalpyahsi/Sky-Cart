...
function filterProducts() {
    ...
    // Rating filtresi - aralıklı filtreleme
    if (ratingValue > 0) {
        filteredProducts = filteredProducts.filter(product => {
            if (ratingValue === 4) {
                return product.rating >= 4.0;
            } else if (ratingValue === 3) {
                return product.rating >= 3.0 && product.rating < 4.0;
            } else if (ratingValue === 2) {
                return product.rating >= 2.0 && product.rating < 3.0;
            } else if (ratingValue === 1) {
                return product.rating >= 0.0 && product.rating < 2.0;
            }
            return true;
        });
    }
